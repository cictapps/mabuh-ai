    ...
    if (listBuffer.type === "bullet") {
      ...
      elements.push(
        <ol key={key} className="mt-1 mb-1 space-y-1 pl-1 list-none">
          {listBuffer.items.map((item, i) => (
            <li key={i} className="flex items-start gap-2">
              <span className="shrink-0 font-semibold opacity-60 text-xs mt-0.5">
                {i + 1}.
              </span>
              <span>{parseInline(item)}</span>
            </li>
  ...
  <div className="flex gap-1.5 px-1 py-2">
    {[0, 1, 2].map((i) => (
      <motion.div
        ...
        animate={isMaskMode ? { opacity: [0.2, 0.5, 0.2] } : { opacity: [0.3, 1, 0.3] }}
        transition={{
...
export const ChatBubble = ({
  ...
  const rendered = isAi && typeof message === "string" ? parseMarkdown(message) : message;
