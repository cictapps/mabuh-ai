...
export type JourneyStatus = {
  currentCheckpoint: JourneyCheckpoint | null;
...
export function normalizeCheckpointTime(value: string): string | null {
...
export function parseCheckpointTime(
  value: string,
...
export function getJourneyStatus(
  ...
  let currentCheckpoint: JourneyCheckpoint | null = null;
  let nextCheckpoint: JourneyCheckpoint | null = null;
  ...
  if (currentDate && nextDate) {
    ...
    progressPercent =
      legDuration > 0
        ? Math.min(100, Math.max(0, Math.round((elapsed / legDuration) * 100)))
        : 100;
  }
